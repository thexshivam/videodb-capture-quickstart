...
class VideoDBService:
    ...
    def _get_connection(self, api_key: str = None) -> videodb.Connection:
        ...
        if key not in self._connections:
            # Only pass base_url if explicitly set, otherwise SDK uses its default
    ...
    def create_capture_session(
        ...
        Args:
            ...
            ws_connection_id: Optional WebSocket connection ID for capture events
        ...
        try:
            ...
            logger.info(f"Creating capture session for user: {end_user_id}, ws_connection_id: {ws_connection_id}")

            # Create capture session with optional ws_connection_id
